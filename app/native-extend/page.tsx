...
import { getSizeNative, initActix, inputImage, inputImageExtend } from "../hooks/native/func";
...
export default function Native() {
	...
	const [sizeType, setSizeType] = useState<string>("FHD");
	...
	async function inputImageTarget(e: React.ChangeEvent<HTMLInputElement>) {
		if (!actixInitialized) {
			...
			setIsUpload(true);
			const { imgUrl, imgArr } = await inputImageExtend(e, sizeType);
	...
	return (
		<div
			...
			<DndContext onDragStart={closeWelcomeOverlay} onDragEnd={handleDragEnd}>
				...
				{!hook.imgUrl && (
					<Select onValueChange={handleSizeTypeChange}>
						<SelectTrigger className="fixed top-4 left-4 w-20 z-10">
